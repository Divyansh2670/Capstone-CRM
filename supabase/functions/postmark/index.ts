...
import { serve } from "jsr:@supabase/functions"; // Import serve from Supabase Edge Functions

// Move env variable access inside the handler using context.env
serve(async (req: Request, context: { env: { POSTMARK_WEBHOOK_USER: any; POSTMARK_WEBHOOK_PASSWORD: any; POSTMARK_WEBHOOK_AUTHORIZED_IPS: any; }; }) => {
  const webhookUser = context.env.POSTMARK_WEBHOOK_USER;
  const webhookPassword = context.env.POSTMARK_WEBHOOK_PASSWORD;
  ...
  const rawAuthorizedIPs = context.env.POSTMARK_WEBHOOK_AUTHORIZED_IPS;
  if (!rawAuthorizedIPs) {
  ...
  response = checkRequestTypeAndHeaders(req, rawAuthorizedIPs, webhookUser, webhookPassword);
